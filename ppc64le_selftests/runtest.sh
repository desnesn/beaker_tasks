...
rlJournalStart
	...
	rlPhaseStartTest
		...
		grep "^ok" $SELFTESTSLOG | awk '{print $3$4" "$5}' >> $SELFTESTSPASS
		grep "^not ok" $SELFTESTSLOG | awk '{print $4$5" "$6" "$7$8" "$9}' >> $SELFTESTSFAIL
		grep "skip:" $SELFTESTSLOG | awk '{print $3}' >> $SELFTESTSSKIP
